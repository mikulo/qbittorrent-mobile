...
**当前源码版本：0.3.6 · Android API 26+ · Java 17 · GPL-3.0-or-later**
...
| 刷新 | 列表、详情、通知和后台状态采样约 500ms；事件合并和离页停止渲染 |
...
当前 instrumentation 有 5 个测试方法，分别覆盖：

- 1,000 文件的添加窗口、固定确认区、选择保留和限速传递。
- 事件洪峰合并、生命周期停止、缓存读取及半秒刷新容差。
...
- Piece 内已下载量当前使用 payload 增量估计，不等于已校验落盘字节；重传等情况下可能高估。
